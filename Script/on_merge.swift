...
//
//  Created by Piotr Jeremicz on 6.11.2025.
//
...
struct Submission {
    ...
    var row: String {
        ...
        let technologiesRow = technologies.prefix(4).joined(separator: ", ")
...
##### Total: \(submissions.count) | Awarded: \(submissions.filter { $0.status == .accepted || $0.status == .distinguished || $0.status == .winner }.count)
"""
}
